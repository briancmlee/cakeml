...
val basis_ffi_oracle_def = Define `
                                  basis_ffi_oracle =
\name (cls,fs,ts) conf bytes.
  if name = ExtCall "write" then
    case ffi_write conf bytes fs of
    | SOME(FFIreturn bytes fs) => Oracle_return (cls,fs,ts) bytes
    ...
    if name = ExtCall "read" then
      ...
      if name = ExtCall "get_arg_count" then
        ...
        | SOME(FFIreturn bytes cls) => Oracle_return (cls,fs,ts) bytes
        | _ => Oracle_final FFI_failed else
...
[mk_proj2 cl_ffi_part;
 ...
 mk_proj2 ts_ffi_part;
 mk_proj2 runtime_ffi_part]`;

Theorem basis_proj1_write:
  basis_proj1 ffi ' "write" = encode(FST (SND ffi))
Proof
...
Theorem extract_fs_basis_ffi:
  !ll. extract_fs fs (basis_ffi cls fs ts).io_events =
       decode (basis_proj1 (basis_ffi cls fs ts).ffi_state ' "write")
Proof
...
Theorem whole_prog_spec2_semantics_prog:
  ∀fname fv.
  Decls env1 (init_state (basis_ffi cl fs ts) with eval_state := es) prog env2 st2 ==>
  lookup_var fname env2 = SOME fv ==>
  whole_prog_spec2 fv cl fs ts sprop Q ==>
  (?h1 h2. SPLIT (st2heap (basis_proj1, basis_proj2) st2) (h1,h2) /\
           (COMMANDLINE cl * STDIO fs * TIME ts * case sprop of NONE => &T | SOME Q => Q) h1)
  ==>
  ∃io_events fs'.
  semantics_prog (init_state (basis_ffi cl fs ts) with eval_state := es) env1
...
Theorem whole_prog_spec_semantics_prog:
  ...
  Decls env1 (init_state (basis_ffi cl fs ts) with eval_state := es) prog env2 st2 ==>
  lookup_var fname env2 = SOME fv ==>
  whole_prog_spec fv cl fs ts sprop Q ==>
  (?h1 h2. SPLIT (st2heap (basis_proj1, basis_proj2) st2) (h1,h2) /\
           (COMMANDLINE cl * STDIO fs * TIME ts * case sprop of NONE => &T | SOME Q => Q) h1)
  ==>
  ∃io_events fs'.
...
Theorem whole_prog_spec_semantics_prog_ffidiv:
   ∀fname fv.
     Decls env1 (init_state (basis_ffi cl fs ts) with eval_state := es) prog env2 st2 ==>
     lookup_var fname env2 = SOME fv ==>
     whole_prog_ffidiv_spec fv cl fs ts Q ==>
     (?h1 h2. SPLIT (st2heap (basis_proj1, basis_proj2) st2) (h1,h2) /\
     (COMMANDLINE cl * STDIO fs * TIME ts * RUNTIME) h1)
   ==>
   ∃io_events fs' n c b.
     semantics_prog (init_state (basis_ffi cl fs ts) with eval_state := es) env1
       (SNOC ^main_call prog)
...
Proof
  ...
  \\ disch_then (qspec_then `COMMANDLINE cl * STDIO fs * TIME ts * RUNTIME` mp_tac)
  \\ simp[] \\ strip_tac
  \\ asm_exists_tac \\ rw[]
  \\ rw[extract_fs_def,PULL_EXISTS]
  \\ drule RTC_call_FFI_rel_IMP_basis_events
  ...
  \\ rveq
  \\ `∃ll. FST (SND st3.ffi.ffi_state) = fs' with numchars := ll` suffices_by ( rw[] \\ rw[] )
  \\ fs[STDIO_def, IOFS_def,cfHeapsBaseTheory.IO_def,
        cfHeapsBaseTheory.IOx_def, set_sepTheory.SEP_CLAUSES,
        set_sepTheory.SEP_EXISTS_THM, fsFFITheory.fs_ffi_part_def]
  \\ fs[GSYM set_sepTheory.STAR_ASSOC]
...
val basis_ffi_part_defs = save_thm("basis_ffi_part_defs",
  LIST_CONJ
    [fs_ffi_part_def,clFFITheory.cl_ffi_part_def,runtime_ffi_part_def, ts_ffi_part_def]);
...
Proof
  ...
  \\ TRY (
     ...
     \\ CASE_TAC \\ fs[fmap_eq_flookup,FLOOKUP_UPDATE]
     \\ rw[] )
  \\ fs[cl_ffi_no_ffi_div,fs_ffi_no_ffi_div,ts_ffi_no_ffi_div]
  \\ disj2_tac
  \\ CCONTR_TAC \\ fs[] \\ rfs[]
QED
